    ...
    const phasesHtml = r.phases
      .map((p, i) => {
        ...
        const empty = !p.ingredients && !p.instructions
          ...
          : "";
        return `<div class="phase"><div class="phase-header">Phase ${i + 1}: ${p.name}</div>${ingHtml}${insHtml}${empty}</div>`;
      })
      .join("");
    return `<!DOCTYPE html><html><head><meta charset="utf-8"/><title>${r.name}</title><style>*{box-sizing:border-box}body{font-family:-apple-system,Helvetica,sans-serif;margin:0;padding:24px;color:#111;font-size:13px;line-height:1.6}h1{font-size:22px;margin:0 0 4px;font-weight:700}h2{font-size:14px;font-weight:600;margin:20px 0 10px;color:#555}.meta{color:#888;font-size:12px;margin:0 0 20px}.phase{border:1px solid #ddd;border-radius:8px;padding:14px 16px;margin-bottom:12px}.phase-header{font-size:14px;font-weight:600;margin-bottom:8px}.recipe-info{margin:0 0 10px}.recipe-label{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:#999;display:block;margin-bottom:3px}.recipe-text{margin:0;white-space:pre-wrap}.recipe-empty{color:#bbb;font-style:italic;margin:0}.footer{margin-top:24px;color:#aaa;font-size:11px;text-align:center}@media print{body{padding:16px}.phase{break-inside:avoid}}</style></head><body><h1>${r.name}</h1><p class="meta">${r.phases.length} ${r.phases.length === 1 ? "phase" : "phases"} · Created ${date}</p><h2>Phases</h2>${phasesHtml}<p class="footer">Bread Lab · ${new Date().toLocaleDateString()}</p></body></html>`;
  };
    ...
    const phasesHtml = bake.phases
      .map((p, i) => {
        ...
        const insHtml = p.instructions
          ? `<div class="recipe-info"><span class="recipe-label">Instructions</span><p class="recipe-text">${p.instructions.replace(/\n/g, "<br>")}</p></div>`
          : "";
        return `<div class="phase"><div class="phase-header"><span class="phase-status">${status}</span> Phase ${i + 1}: ${p.name} <span class="dur">${dur}</span></div>${ingHtml}${insHtml}${volLine}${readingsHtml}</div>`;
      })
      .join("");
